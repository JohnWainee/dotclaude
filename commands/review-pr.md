...
Provide an **adversarial** code review for the given pull request. Posture:
guilty until proven innocent — hunt for the defect, do not bless the change.

**Before anything else**, read the adversarial review checklist at
`~/.claude/references/adversarial-review-checklist.md` (honor `$CLAUDE_CONFIG_DIR`
if set). Its operating rules, severity taxonomy, and anti-pattern catalog are
**binding** for every finder and scorer agent below — pass the checklist content
to each agent verbatim.
...
6. Filter out any issues with a score less than 80 (matches the checklist's ≥80 gate). If there are no issues that meet this criteria, do not proceed.
7. Use a Haiku agent to repeat the eligibility check from #1, to make sure that the pull request is still eligible for code review.
8. Finally, use the gh bash command to comment back on the pull request with the result. When writing your comment, keep in mind to:
   a. Keep your output brief
   b. Avoid emojis
   c. Link and cite relevant code, files, and URLs
   d. Lead each finding with its severity label and the failing scenario (per the checklist output shape) — no praise, no hedging.
...
- General code quality issues (eg. lack of test coverage, general security issues, poor documentation), unless explicitly required in CLAUDE.md or surfaced as a defensible checklist finding
...
1. <severity> <brief description of bug> (CLAUDE.md says "<...>")

<link to file and line with full sha1 + line range for context, note that you MUST provide the full sha and not use bash here, eg. https://github.com/anthropics/claude-code/blob/1d54823877c4de72b2316a64032a54afc404e619/README.md#L13-L17>

2. <severity> <brief description of bug> (some/other/CLAUDE.md says "<...>")

<link to file and line with full sha1 + line range for context>

3. <severity> <brief description of bug> (bug due to <file and code snippet>)
...
No defensible findings ≥80 confidence. Checked for bugs, checklist anti-patterns, and CLAUDE.md compliance.
